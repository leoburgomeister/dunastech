...
import { buildStyleUrl, apply3DScene } from '@/lib/map/scene3d';
import {
  rnOverview,
  destinationCamera,
  flightDuration,
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      if (is3D(mapMode) && maptilerKey) {
        try {
          // Mercator: o hero fecha em um destino so, entao o globo nao agrega
          // e ainda quebra o relevo (ver Scene3DOptions).
          apply3DScene(map, maptilerKey, { projection: 'mercator' });
        } catch (e) {
