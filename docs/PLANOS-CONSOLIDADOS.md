...
| B5 | ~~Suspensão de atrativo pela IGR usando `destinos.status`~~ — **RESOLVIDO em 10/09**: `destinos_update_admin` (RLS de update gated por `usuarios.role = 'admin'`, migration `0011`) fecha o elo que faltava — a sessão Supabase já existia desde a 0006, só não havia policy de escrita. `supabase-data.ts` agora lê `status`; `gestao/destinos` tem botão de suspender/reativar (`setDestinoStatus`, escrita direta pela sessão do admin); `route-planner` exclui do catálogo todo destino com status diferente de `ATIVO` (não é peso do ISA, é corte). Não existe papel de IGR dedicado (`usuarios.role` só tem `tourist`/`admin` — ver B13) — a ação usa o `admin` existente até a multi-tenancy por região chegar | spec ISA §Próximos passos |
...
| B9 | ~~Catálogo intermediário (nada entre Maracajaú e Gostoso; 246 km sem parada até Mossoró)~~ — **RESOLVIDO em 10/09**: novo destino "Farol de Touros" (Ponta do Calcanhar, município Touros) cadastrado a meio caminho — 28,9 km de Maracajaú, 21,5 km de Gostoso, ambos dentro do alcance de buggy/van — em `mockData.ts`, `route-planner.ts` (afinidade de adventure/relax/ecotourism/culture/gastronomy) e `supabase/migrations/0010_farol_de_touros.sql`. Os 246 km Forte→Mossoró da "Grande Rota Histórica" continuam intencionais (a cópia nomeia os dois destinos, ver B10) | spec alcance §Próximos passos |
...
| B11 | ~~Reavaliar teto de buggy (12) se o catálogo do litoral norte crescer~~ — **RESOLVIDO em 10/09**: medido com `planRoute` contra o catálogo real (com e sem Farol de Touros/B9). Sem o Farol de Touros o preenchimento de buggy esgotava em 12 destinos alcançáveis (`comfortableLegKm` 60 km); com ele, esgota em 13 — dia 13 vira destino genuíno (28,9 km de pior dia), dia 14+ vira permanência. Teto subiu de 12 para 13 em `MAX_DIAS_POR_TRANSPORTE` (`routePresets.ts`). A justificativa antiga (evitar o escape `escolher(Infinity)`, 189 km) já não se aplica desde B10: a barreira é dura, então dia sem destino novo vira permanência, nunca distância absurda — o teto agora só marca o último dia que rende destino real | spec alcance §Próximos passos, B9, B10 |
| B12 | ~~Roteiros 13–15 dias: alvo encosta no catálogo e o ISA deixa de influenciar~~ — **RESOLVIDO em 10/09, por acúmulo de outras correções**: a limitação foi escrita em 29/07 contra `perDay` 1,5 (buggy/shuttle) e catálogo de 20. `perDay` voltou a 1 em todo transporte no dia seguinte (30/07, decisão de produto não relacionada) e o catálogo cresceu para 21 com o Farol de Touros (B9, 10/09). Com as duas contas atuais, o teto de shuttle (15 dias) usa 15 dos 21 destinos — o pool nunca esgota entre 1 e 15 dias, então o ISA continua decidindo quem entra. Trancado em teste de regressão (`route-planner.test.ts`, "em 13-15 dias o catálogo não esgota"); suíte 270 verde. Sem mudança de comportamento — só verificação e trava | spec ISA §Limitação |
